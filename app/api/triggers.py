...
class DemoSimulateTriggerRequest(BaseModel):
    rider_id: str = "rdr_demo_hsr"
    zone: str = "HSR Layout"
    trigger_type: str = "HEAVY_RAIN"
    trigger_value: Optional[float] = None
...
def demo_simulate_trigger(payload: DemoSimulateTriggerRequest, db: Session = Depends(get_db)):
    trigger_type = payload.trigger_type.upper()
    if trigger_type not in TRIGGER_THRESHOLDS:
        raise HTTPException(
            status_code=422,
            detail={"error": "UNSUPPORTED_TRIGGER", "message": "unsupported trigger_type"},
        )

    # Derived trigger value or fallback
    derived_value = payload.trigger_value
    if derived_value is None:
        weather = WeatherService.get_current_conditions(payload.zone, is_simulated=True)
    ...
    from app.services.payout_service import PayoutService, TriggerEvent
    
    event = TriggerEvent(
        trigger_type=trigger_type,
        value=float(derived_value),
        threshold=float(result["trigger_event"]["threshold"]),
        breached=result["trigger_event"]["breached"]
    )
    
    payout_res = PayoutService.process_trigger_payout(
        rider_id=payload.rider_id,
        trigger_event=event,
        fraud_result=result,
        db=db
    ...
    return {
        ...
        "payout_amount": payout_res.payout_amount,
        "utr_number": payout_res.utr_number,
