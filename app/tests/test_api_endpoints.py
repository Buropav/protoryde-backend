...
class TestApiEndpoints(unittest.TestCase):
    ...
    def test_demo_bootstrap_success(self):
        payload = {
        ...
        response = self.client.post("/api/demo/bootstrap", json=payload)
        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertEqual(body["status"], "ok")
        self.assertEqual(body["rider"]["rider_id"], payload["rider_id"])
        self.assertIn("policy_id", body["policy"])

    def test_demo_bootstrap_exclusions_guard(self):
        payload = {
    ...
    def test_simulate_returns_contract_shape(self):
        ...
        self.client.post("/api/demo/bootstrap", json=bootstrap)
    ...
    def test_policy_pdf_download_headers(self):
        ...
        response = self.client.post("/api/demo/bootstrap", json=bootstrap)
        self.assertEqual(response.status_code, 200)

        pdf_response = self.client.get(f"/api/policies/{rider_id}/current/document")
        self.assertEqual(pdf_response.status_code, 200)
        self.assertEqual(pdf_response.headers["content-type"], "application/pdf")
        self.assertIn("attachment;", pdf_response.headers.get("content-disposition", ""))
