...
def generate_ledger_pdf(
    ...
    story.append(
        _table(
            [
                ["Total Base Premium Paid", f"INR {summary_metrics.get('total_base_premium', 0.0):.2f}"],
                ["Total Claims Paid Out", f"INR {summary_metrics.get('total_claims_paid', 0.0):.2f}"],
                ["Net Balance Paid Out", f"INR {summary_metrics.get('net_balance', 0.0):.2f}"],
    ...
    if policies:
        ...
        for p in policies:
            ...
            start_str = start_dt.strftime("%d %b %Y") if isinstance(start_dt, datetime) else str(start_dt)[:10]
            policy_rows.append([
                str(p.get("id", ""))[:12] + "...",
                start_str,
        ...
        story.append(_table(policy_rows, header=True, col_widths=[45 * mm, 40 * mm, 40 * mm, 40 * mm]))
    else:
        story.append(Paragraph("No policies found in the last 12 months.", styles["body"]))
    ...
    if claims:
        ...
        story.append(_table(claim_rows, header=True, col_widths=[45 * mm, 60 * mm, 30 * mm, 30 * mm]))
    else:
        story.append(Paragraph("No claims filed in the last 12 months.", styles["body"]))
